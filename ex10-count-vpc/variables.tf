variable "ec2_instance_config_list" {
  ...
  validation {
    condition     = alltrue([for config in var.ec2_instance_config_list : contains(["ubuntu", "amazon_linux"], config.ami)])
    error_message = "Only \"ubuntu\" and \"amazon_linux\" AMIs are allowed."
  }
}
...
variable "ec2_instance_config_map" {
  ...
  validation {
    condition     = alltrue([for config in values(var.ec2_instance_config_map) : contains(["ubuntu", "amazon_linux"], config.ami)])
    error_message = "Only \"ubuntu\" and \"amazon_linux\" AMIs are allowed."
  }
}
